...
class CustomDioLogger extends Interceptor {
  ...
  CustomDioLogger({
    ...
    this.compact = true,
    this.logPrint = print,
    this.exceptionEndpoints,
  });

  bool checkExceptions(String path) {
    if (exceptionEndpoints == null || exceptionEndpoints!.isEmpty) {
      return false;
    }
    return exceptionEndpoints!.any((e) => path.contains(e));
  }

  @override
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    if (checkExceptions(options.path)) {
      return super.onRequest(options, handler);
    }
    logPrint = getOneLogger().i;
    if (request) {
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) {
    if (checkExceptions(response.requestOptions.path)) {
